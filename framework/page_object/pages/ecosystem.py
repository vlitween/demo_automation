...
from framework.page_object.locators.ecosystem import EcosystemLocators
...
class EcosystemPage(BasePage):
    url = f'{variables.MAIN_URL}/ecosystem/'

    @property
    def locators(self):
        return EcosystemLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('ecosystem.page_title'))

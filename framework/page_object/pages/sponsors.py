...
from framework.page_object.locators.sponsors import SponsorsLocators
...
class SponsorsPage(BasePage):
    url = f'{variables.MAIN_URL}/sponsors/'

    @property
    def locators(self):
        return SponsorsLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('sponsors.page_title'))

...
from framework.page_object.locators.events import EventsLocators
...
class EventsPage(BasePage):
    url = f'{variables.MAIN_URL}/events/'

    @property
    def locators(self):
        return EventsLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('events.page_title'))

...
from framework.page_object.locators.sponsor_us import SponsorUsLocators
...
class SponsorUsPage(BasePage):
    url = f'{variables.MAIN_URL}/sponsor/'

    @property
    def locators(self):
        return SponsorUsLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('sponsor_us.page_title'))

...
from framework.page_object.locators.structure_and_governance import \
    StructureAndGovernanceLocators
...
class StructureAndGovernancePage(BasePage):
    url = f'{variables.MAIN_URL}/project/'

    @property
    def locators(self):
        return StructureAndGovernanceLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('structure_and_governance.page_title'))

...
from framework.page_object.locators.get_involved import GetInvolvedLocators
...
class GetInvolvedPage(BasePage):
    url = f'{variables.MAIN_URL}/getinvolved/'

    @property
    def locators(self):
        return GetInvolvedLocators(self.device)
    ...
    def check_page_presence(self, fast_check=False):
        with allure.step('Wait page title'):
            self.actions.wait_page_title(self.translator.get_translation('get_involved.page_title'))
